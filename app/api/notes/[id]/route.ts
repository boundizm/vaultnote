...
export async function GET(
  ...
  try {
    ...
    return NextResponse.json({
      title: note.titleEncrypted ? await decryptMetadata(note.titleEncrypted.toString()) : (note.title || null),
      ciphertext: note.ciphertext.toString('base64'),
      iv: note.iv.toString('base64'),
      remainingReadsPreview,
      isProtected: note.isProtected,
      encryptedKey: note.encryptedKey ? note.encryptedKey.toString('base64') : null,
      keyIv: note.keyIv ? note.keyIv.toString('base64') : null,
      salt: note.salt ? note.salt.toString('base64') : null,
      images: note.images ? JSON.parse(note.images) : null,

      // Author information (decrypted from encrypted fields or fallback to old fields)
      authorName: note.authorNameEncrypted ? await decryptMetadata(note.authorNameEncrypted.toString()) : (note.authorName || 'Anonymous'),
      authorEmail: note.authorEmailEncrypted ? await decryptMetadata(note.authorEmailEncrypted.toString()) : (note.authorEmail || ''),
